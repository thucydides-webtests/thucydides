...
import net.thucydides.samples.MockOpenPageWithWebdriverErrorSample
import net.thucydides.samples.MockOpenStaticDemoPageWithFailureSample
...
import net.thucydides.samples.SamplePassingScenarioUsingHtmlUnitForOneTest
...
import net.thucydides.samples.SamplePendingScenario
import net.thucydides.samples.SampleScenarioWithFailingNestedNonStepMethod
import net.thucydides.samples.SampleScenarioWithFailingNonStepMethod
import net.thucydides.samples.SampleScenarioWithStateVariables
import net.thucydides.samples.SingleHtmlUnitTestScenario
...
import net.thucydides.samples.SingleTestScenarioWithRuntimeException
...
import static org.hamcrest.MatcherAssert.assertThat
import static org.hamcrest.Matchers.is
import net.thucydides.samples.SimpleNonWebScenario
...
class WhenRunningTestScenarios extends Specification {
    ...
    def "steps with multiple parameters should contain the parameter values in the description"() {
        ...
        then:
            firstStep.description == "Step with two parameters: <span class='parameters'>foo, 2</span>"
    }
    ...
    class TestableThucydidesRunner extends ThucydidesRunner {
        TestableThucydidesRunner(Class<?> klass, WebDriverFactory webDriverFactory) {
     ...
     def "xml test results should be written to the output directory"() {
        given:
            def runner = new TestableThucydidesRunner(SamplePassingScenario, webDriverFactory)
        when:
            runner.run(new RunNotifier())
    ...
    def "tests for multiple stories should be written to the output directory"() {
        when:
            new TestableThucydidesRunner(SamplePassingScenarioUsingHtmlUnit, webDriverFactory).run(new RunNotifier())
            new TestableThucydidesRunner(SampleFailingScenarioUsingHtmlUnit, webDriverFactory).run(new RunNotifier())
            def xmlReports = temporaryDirectory.list().findAll {it.endsWith(".xml")}
        then:
            xmlReports.size() == 6
