 ...
 * 
 */
public class WhenRunningTestsWithAManagedWebDriver extends
    ...
    public void the_tests_should_be_executed_in_the_order_of_appearance()
        ...
        final List<String> expectedTestOrder = Arrays.asList(
                "should_do_this_step_1", "should_do_that_step_2",
                "then_gets_here_step_3", "finally_gets_here_step_4",
    ...
    @Test
    public void should_take_a_screenshot_after_each_test()
        ...
        verify(mockPhotographer).takeScreenshot(
                startsWith("should_do_this_step_1"));
        verify(mockPhotographer).takeScreenshot(
                startsWith("should_do_that_step_2"));
        verify(mockPhotographer).takeScreenshot(
                startsWith("then_gets_here_step_3"));
        verify(mockPhotographer).takeScreenshot(
                startsWith("finally_gets_here_step_4"));
        verify(mockPhotographer).takeScreenshot(
                startsWith("and_at_the_end_step_5"));
    }

    @Test
    public void should_not_take_a_screenshot_for_skipped_tests()
            throws InitializationError, IOException {

        TestableWebDriverFactory mockBrowserFactory = new TestableWebDriverFactory();
        ThucydidesRunner runner = getTestRunnerUsing(ManagedWebDriverSampleWithFailingTest.class, mockBrowserFactory);
