...
class WhenLoadingCapabilitiesFromADirectoryStructure extends Specification {
    ...
    def "Should be able to load capabilities from the default directory structure"() {
        given: "We are using the default capabilities provider"
            CapabilityProvider capabilityProvider = new FileSystemCapabilityProvider("sample-story-directories/capabilities_and_features");
        when: "We load the available capabilities"
            def capabilities = capabilityProvider.getCapabilities()
            def capabilityNames = capabilities.collect {it.name}
        then: "the capabilities should be loaded from the first-level sub-directories"
    ...
    def "Should derive title from directory name if not present in narrative"() {
        given: "there is a capability.narrativeText file in a directory that does not have a title line"
            def capabilityProvider = new FileSystemCapabilityProvider("sample-story-directories/capabilities_and_features")
    ...
    def "the capability type is derived from the .narrative file name"() {
        ...
        when: "We load the available capabilities"
            def capabilities = capabilityProvider.getCapabilities()
        then: "the capabilities should be of type 'capability"
    ...
    def "capabilities can be nested"() {
        given: "We are using the default capabilities provider"
            CapabilityProvider capabilityProvider = new FileSystemCapabilityProvider("sample-story-directories/capabilities_and_features");
        when: "We load capabilities with nested capability directories"
            def capabilities = capabilityProvider.getCapabilities()
        then: "the nested capabilities should be recorded"
    ...
    def "default nested capability types can be overriden using an environment variable"() {
        given: "We are using the default capabilities provider"
            ...
            FileSystemCapabilityProvider capabilityProvider = new FileSystemCapabilityProvider("sample-story-directories/capabilities_and_features", 0, vars);
